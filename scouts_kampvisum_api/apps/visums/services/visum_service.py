...
class CampVisumService:
    ...
    def visum_create(self, request, **data) -> CampVisum:
        ...
        year = data.get("year", self.year_service.get_current_camp_year())
        ...
        logger.debug(f"Creating camp '{name}' for group {group.group_admin_id}")
        ...
        logger.debug("Creating LinkedCategorySet for visum %s", visum.name)
        category_set: LinkedCategorySet = self.category_set_service.create_linked_category_set(
        ...
        logger.info("CampVisum created %s (%s)", visum.name, visum.id, user=request.user)
    ...
    def visum_update(self, request, instance: CampVisum, **fields) -> CampVisum:
        ...
        category_set: LinkedCategorySet = self.category_set_service.update_linked_category_set(
            request=request,
    ...
    def delete_visum(self, request, instance: CampVisum):
        logger.debug("Deleting CampVisum with id %s for camp %s", instance.id, instance.name)

        engagement: CampVisumEngagement = instance.engagement

        self.linked_deadline_service.delete_linked_deadlines_for_visum(request=request, visum=instance)
