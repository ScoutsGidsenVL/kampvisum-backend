...
class CampVisum(GroupAdminIdMixin, GroupNameMixin, AuditedBaseModel):
    ...
    class Meta:
        ...
        permissions = [
            ...
            ("view_campvisum_member_data", "User is allowed to view gdpr-sensitive data of members"),
    ...
    def location(self):
        ...
        location = None
        logistics = LinkedCategory.objects.filter(category_set__id=self.category_set.id, parent__name="logistics")
                ...
                for linked_location_check in linked_location_checks:
                    for linked_location in linked_location_check.locations.all():
                        for camp_location in CampLocation.objects.filter(location_id=linked_location.id):
                            location = CampLocationMinimalSerializer(camp_location, many=False).data
