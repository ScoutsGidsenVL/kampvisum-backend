...
class LinkedDeadlineViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request):
        ...
        input_serializer = LinkedDeadlineInputSerializer(data=request.data, context={"request": request})
        ...
        instance = self.linked_deadline_service.create_or_update_linked_deadline(request, **validated_data)

        output_serializer = LinkedDeadlineSerializer(instance, context={"request": request})

        return Response(output_serializer.data, status=status.HTTP_201_CREATED)

    @swagger_auto_schema(responses={status.HTTP_200_OK: LinkedDeadlineSerializer})
    def retrieve(self, request, pk=None):
        instance: LinkedDeadline = LinkedDeadline.objects.safe_get(pk=pk, raise_error=True)
    ...
    def partial_update(self, request, pk):
        ...
        instance: LinkedDeadline = self.linked_deadline_service.get_linked_deadline(linked_deadline_id=pk)
        ...
        logger.debug("LINKED DEADLINE UPDATE VALIDATED DATA: %s", validated_data)

        instance = self.linked_deadline_service.update_linked_deadline(request, instance, **validated_data)
    ...
    def list(self, request):
        ...
        if page is not None:
            serializer = LinkedDeadlineSerializer(page, many=True, context={"request": request})
            return self.get_paginated_response(serializer.data)
        else:
            serializer = LinkedDeadlineSerializer(instances, many=True, context={"request": request})
            return Response(serializer.data)
    ...
    def list_for_visum(self, request, visum_id):
        ...
        instances = self.filter_queryset(self.linked_deadline_service.list_for_visum(visum=visum_id))
        page = self.paginate_queryset(instances)

        if page is not None:
            serializer = VisumDeadlineSerializer(page, many=True, context={"request": request})
            return self.get_paginated_response(serializer.data)
        else:
            serializer = VisumDeadlineSerializer(instances, many=True, context={"request": request})
            return Response(serializer.data)
    ...
    def partial_update_linked_deadline_flag(self, request, linked_deadline_id, linked_deadline_flag_id):
        ...
        validated_data = serializer.validated_data
        logger.debug("LINKED DEADLINE FLAG UPDATE VALIDATED DATA: %s", validated_data)

        instance: LinkedDeadlineFlag = self.linked_deadline_flag_service.update_linked_deadline_flag(
            request, instance, **validated_data
        )

        instance: LinkedDeadline = self.linked_deadline_service.get_visum_deadline(linked_deadline=linked_deadline_id)
