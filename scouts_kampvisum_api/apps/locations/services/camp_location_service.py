...
class CampLocationService:
        ...
        **data,
        ...
        if instance:
            ...
            if not linked_location:
                raise ValidationError("Unable to find LinkedLocation instance with id {}".format(linked_location.id))
            ...
            if id:
                ...
                if not linked_location:
                    raise ValidationError(
                        "Unable to find LinkedLocation instance with id {}".format(linked_location.id)
                    )

        if linked_location:
            linked_location = self._update_linked_location(
                request=request, instance=linked_location, is_camp_location=is_camp_location, **data
            )
            ...
            linked_location = self._create_linked_location(request=request, is_camp_location=is_camp_location, **data)

            check.locations.add(linked_location)

        check.center_latitude = (
            linked_location.center_latitude if linked_location.center_latitude else check.center_latitude
        )
        check.center_longitude = (
            linked_location.center_longitude if linked_location.center_longitude else check.center_longitude
        )
        ...
        existing_locations = [location.id for location in linked_location.locations.all()]
        logger.debug("LINKED LOCATION CREATE OR UPDATE DATA: %s", data)

        if not linked_location_provided:
            locations = data.get("locations", [])
            posted_locations = [uuid.UUID(location.get("id")) for location in locations if location.get("id", None)]
    ...
    @transaction.atomic
    def _create_linked_location(self, request, is_camp_location: bool = False, **data) -> LinkedLocation:
    ...
    def _update_linked_location(
        ...
        instance.center_longitude = data.get("center_longitude", instance.center_longitude)
        instance.zoom = data.get("zoom", None)
    ...
    def create_or_update_camp_location(
        ...
        if id:
            ...
            if not location:
                raise ValidationError(
                    "No CampLocation found with id {} for LinkedLocationCheck with id {}".format(id, instance.id)
                )

            return self.update_camp_location(request=request, instance=location, location=instance, **data)
        else:
            return self.create_camp_location(request=request, location=instance, **data)

    @transaction.atomic
    def create_camp_location(self, request, location: LinkedLocation, **data) -> CampLocation:
    ...
    @transaction.atomic
    def update_camp_location(self, request, instance: CampLocation, location: LinkedLocation, **data) -> CampLocation:
        logger.debug("LOCATION SERVICE UPDATE DATA: %s", data)

        instance.location = location
        instance.name = data.get("name", instance.name)
        instance.address = data.get("address", instance.address)
        instance.is_main_location = data.get("is_main_location", instance.is_main_location)
    ...
    def remove_camp_location(self, instance: LinkedLocation, location: CampLocation):
        logger.debug("Removing camp location %s from location check %s", location.id, instance.id)
